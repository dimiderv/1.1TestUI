                ...
                <div className="d-flex align-items-center 
                  justify-content-center flex-wrap p-2 m-2 ">
                    {items.map((item) => (
                        ...
                        <PrintAssets ID={item.ID} color={item.color} weight={item.weight} owner={item.owner} creator={item.creator} expirationDate={item.expirationDate} sensorData={item.sensorData} ownerOrg={item.ownerOrg} assetType={item.assetType}/>
                    ))}
                </div>
                <div>
